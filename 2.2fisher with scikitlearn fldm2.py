...
import random
#this is used to read the data from the file mixedfisherdata.csv by droping the nan values
data = pd.read_csv("fisherdata.csv").dropna()

# get the column names except the last column
...
data = data[column_names + [data.columns[-1]]]
...
X_train_proj_np = X_train_proj.values.reshape(-1, 1)
